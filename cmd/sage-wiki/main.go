...
func init() {
	...
	compileCmd.Flags().Bool("re-write", false, "Re-write only failed articles (use after --re-extract with errors)")
...
func runCompile(cmd *cobra.Command, args []string) error {
	...
	reWrite, _ := cmd.Flags().GetBool("re-write")
	if reWrite {
		result, err := compiler.ReWrite(dir)
		if err != nil {
			return err
		}
		fmt.Printf("Re-write complete: %d articles, %d errors\n",

...
func buildQueryContext(projectDir string, question string, topK int, cfg *config.Config, db *storage.DB) (string, []string, error) {
	...
	if chunkCount > 0 {
		...
		if cfg.Search.QueryExpansionEnabled() || rerankEnabled {
			client, _ = llm.NewClient(cfg.API.Provider, cfg.API.APIKey, cfg.API.BaseURL, cfg.API.RateLimit, llm.WithExtraParams(cfg.API.ExtraParams), llm.WithTimeout(cfg.API.TimeoutSeconds))
		}
